"""장고 기본 제공 필드가 아닌 직접 작성한 필드 == custom fields"""
...
class ThumbnailImageFieldFile(ImageFieldFile):
    ...
    @property
    def thumb_url(self):
        return self.add_thumb(self.url)

    def save(self, name, content, save=True):
        super().save(name, content, save) # 부모 ImageFieldFile 클래스의 save() 메소드를 호출하여 원본 이미지 저장

        img = Image.open(self.path)
        size = (self.field.thumb_width, self.field.thumb_height) 
        img.thumbnail(size) # PIL 라이브러리의 Image.thumbnail() 함수 사용
